...
def selects_the_titles_and_amount_over_goal_of_all_projects_that_have_met_their_funding_goal
  "SELECT projects.title, (SUM(pledges.amount) - projects.funding_goal) FROM projects INNER JOIN pledges WHERE projects.id = pledges.project_id GROUP BY projects.title HAVING 0 <= SUM(pledges.amount) - projects.funding_goal;"
end

def selects_user_names_and_amounts_of_all_pledges_grouped_by_name_then_orders_them_by_the_summed_amount
  "SELECT users.name, (SELECT SUM(pledges.amount) FROM pledges WHERE pledges.user_id = users.id) as Amount FROM users GROUP BY users.name, Amount ORDER BY Amount" 
end
